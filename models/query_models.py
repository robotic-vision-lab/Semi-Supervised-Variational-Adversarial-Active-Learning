...
class LossNet(nn.Module):
    def __init__(self, feature_sizes=[32, 16, 8, 4], num_channels=[64, 128, 256, 512], interm_dim=128):
        ...
        self.linear = nn.Linear(4 * interm_dim, 1)  # (512, 1)
    ...
    def forward(self, features):
        ...
        """
        out1 = self.GAP1(features[0])  # torch.Size([128, 64, 1, 1])
        out1 = out1.view(out1.size(0), -1)  # torch.Size([128, 64])
        out1 = F.relu(self.FC1(out1))  # torch.Size([128, 128])

        out2 = self.GAP2(features[1])  # torch.Size([128, 128, 1, 1])
        out2 = out2.view(out2.size(0), -1)  # torch.Size([128, 128])
        out2 = F.relu(self.FC2(out2))       # torch.Size([128, 128])

        out3 = self.GAP3(features[2])  # torch.Size([128, 256, 1, 1])
        out3 = out3.view(out3.size(0), -1)  # torch.Size([128, 256])
        out3 = F.relu(self.FC3(out3))     # torch.Size([128, 128])

        out4 = self.GAP4(features[3])  # torch.Size([128, 512, 1, 1])
        out4 = out4.view(out4.size(0), -1)  # torch.Size([128, 512])
        out4 = F.relu(self.FC4(out4))   # torch.Size([128, 128])

        out = self.linear(torch.cat((out1, out2, out3, out4), 1))  # ([128, 512]) -> torch.Size([128, 1])
        return out
...
def kaiming_init(m):
    if isinstance(m, (nn.Linear, nn.Conv2d)):
        ...
        init.kaiming_normal_(m.weight)  # UserWarning: nn.init.kaiming_normal is now deprecated in favor of nn.init.kaiming_normal_.
