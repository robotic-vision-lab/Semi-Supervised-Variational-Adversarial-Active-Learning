...
def train_epoch(models, method, criterion, optimizers, dataloaders, epoch, epoch_loss):
    ...
    for data in tqdm(dataloaders['train'], leave=False, total=len(dataloaders['train'])):
        ...
        if method == 'lloss' or 'TA-VAAL':
            ...
            # ranking_criterion = SpearmanLoss(*load_sorter('weights/best_model.pth.tar')).to(device)
            # ranking_criterion = SpearmanLoss(*load_sorter('weights/best_model_grup_06171954_L0027.pth.tar')).to(device)
